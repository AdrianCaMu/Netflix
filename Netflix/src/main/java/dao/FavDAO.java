...
/**
 * Conexión de la tabla de favoritos de la base de datos
...
public class FavDAO extends AbstractDAO{
	 ...
	 * @param id id del usuario que queremos sus favoritas
	...
	public ArrayList<Show> getAll(int idUser) {
		final String QUERY = "SELECT id, type, title, director, cast, country, date_added, release_year, rating, duration, listed_in, description "
				+ "FROM Series INNER JOIN Favoritos on Favoritos.id_Show = Series.id WHERE Favoritos.id_User = " + idUser +";";
	 ...
	 * @param s show a añadir como favorito
	 * @param id id del usuario
	 */
	public void insert(String idShow, int idUser) {
		final String INSERT = "INSERT INTO Favoritos"
				+ " VALUES('" + idShow + "'," + idUser + ");";
		try {
